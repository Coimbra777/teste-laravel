<template>
      ...
      <table class="agenda-table">
        ...
        <tbody>
          <tr v-for="contact in contacts" :key="contact.id">
            ...
            <td>{{ contact.phone }}</td>
            <td>
              <button class="edit-btn" @click="editContact(contact)">Editar</button>
              <button class="delete-btn" @click="deleteContact(contact.id)">Excluir</button>
            </td>
          </tr>
        </tbody>
      </table>
   ...
   <button class="create-btn" @click="toggleForm">Novo Contato</button>
      ...
      <form @submit.prevent="createContact">
        ...
        <button class="create-btn" type="submit" @click="createContact">Salvar</button>
      </form>
    </div>
  </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      loadContacts() {
      axios.get('http://localhost:8989/api/contacts/')
        .then(response => {
